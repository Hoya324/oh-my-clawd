#!/usr/bin/env node
/**
 * Claude HUD — Status line for Claude Code
 *
 * Display format:
...
function getUsageCachePath() {
  ...
  return join(root, '.claude', '.hud', 'usage-cache.json');
}
...
function render(stdin, transcript, usage) {
  ...
  // 1. [HUD] label
  parts.push(`${BRAND_BOLD}[HUD]${RESET}`);
...
async function main() {
  try {
    ...
    process.stdout.write(`${BRAND_BOLD}[HUD]${RESET}\n`);
  }
}
